...
```
Finally , whenever a write is initiated to a new file we will check if it was previously deleted by the process. If so , we will copy the datapoint stored in the ```DeletedFiles``` list of the process entry structure to the file object context.  We have to make a technical definition as to what a "file" is. Consider a Word document. User opens X.DOCX, deletes some of It, adds some more, and saves it. Is that the same file that he opened? Suppose he saves it with a different name? Is it the same file now? The potential permutations are endless. Since ransomwares have tendency of changing file extensions will filter new files created with the same full name of a previously deleted file by the process , ignroing the extension.
```cpp
